...
public sealed class DataShapingService
{
    ...
    public static List<ExpandoObject> ShapeData<T>(IEnumerable<T> entities, string? fields)
    {
        ...
        if (fieldsSet.Any())
        {
            propertyInfos = propertyInfos
                .Where(p => fieldsSet.Contains(p.Name)).ToArray();
        }
    ...
    public bool IsValidated<T>(string? fields)
    {
